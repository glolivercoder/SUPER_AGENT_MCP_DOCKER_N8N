...
class DatabaseManager:
    ...
    def save_api_key(self, module_name: str, api_key: str, encrypted: bool = True) -> bool:
        ...
        with self.lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    # Se encrypted=True, apenas marca como criptografada (implementar criptografia real depois)
                    value_to_store = f"ENCRYPTED:{api_key}" if encrypted else api_key
                    
                    cursor.execute("""
                        INSERT OR REPLACE INTO configurations (module_name, config_key, config_value, updated_at)
                        VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                    """, (module_name, "api_key", value_to_store))
                    
                    conn.commit()
                    self.logger.info(f"API key salva para módulo: {module_name}")
                    return True
                    
            except Exception as e:
                self.logger.error(f"Erro ao salvar API key: {e}")
                return False

    def get_api_key(self, module_name: str) -> Optional[str]:
        """Recupera API key do banco de dados"""
        with self.lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
                    ...
                    if row:
                        value = row[0]
                        # Remove prefixo de criptografia se existir
                        if value.startswith("ENCRYPTED:"):
                            return value[10:]  # Remove "ENCRYPTED:"
                        return value
                    
                    return None
                    
            except Exception as e:
                self.logger.error(f"Erro ao recuperar API key: {e}")
                return None

    def save_configuration(self, module_name: str, config_key: str, config_value: str) -> bool:
        """Salva configuração genérica no banco"""
        with self.lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
    ...
    def get_configuration(self, module_name: str, config_key: str) -> Optional[str]:
        """Recupera configuração genérica do banco"""
        with self.lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    cursor = conn.cursor()
