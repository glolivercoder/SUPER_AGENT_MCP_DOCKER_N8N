...
class OpenRouterManager:
    ...
    def _load_api_key(self) -> Optional[str]:
        """Carrega API key do banco de dados ou variável de ambiente"""
        # Primeiro tenta do banco de dados
        if self.database_manager:
            api_key = self.database_manager.get_api_key("openrouter")
            if api_key:
                self.logger.info("API key carregada do banco de dados")
                return api_key
        
        # Fallback para variável de ambiente
        api_key = os.getenv("OPENROUTER_API_KEY")
        if api_key:
            self.logger.info("API key carregada da variável de ambiente")
            # Salva no banco para uso futuro
            if self.database_manager:
                self.database_manager.save_api_key("openrouter", api_key)
        
        return api_key

    def set_api_key(self, api_key: str) -> bool:
        """Define nova API key e salva no banco"""
        self.api_key = api_key
        
        # Atualiza OpenAI client
        if self.api_key:
            openai.api_key = self.api_key
            openai.base_url = f"{self.base_url}/chat/completions"
        ...
        if self.database_manager:
            success = self.database_manager.save_api_key("openrouter", api_key)
            if success:
                self.logger.info("API key salva no banco de dados")
                return True
            else:
                self.logger.error("Erro ao salvar API key no banco de dados")
                return False
