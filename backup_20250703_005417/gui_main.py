...
from main import SuperAgent  # Substituir por import absoluto
...
def main():
    ...
    try:
        ...
        try:
            ...
            # Configurar integração de voz
            if agent.voice_module:
                gui.voice_module = agent.voice_module
                # Configurar callback de voz para integrar com a GUI
                def voice_command_handler(command, action):
                    gui.log_text.insert(tk.END, f"Comando de voz: {command} -> {action}\n")
                    gui.log_text.see(tk.END)
                    
                    # Executar ações baseadas no comando
                    if action == "generate_prd":
                        gui.current_agent_type.set("prompt")
                        gui._on_agent_changed()
                        gui.prompt_text.delete(1.0, tk.END)
                        gui.prompt_text.insert(1.0, "Gere um PRD para o projeto atual")
                        gui._send_message()
                    elif action == "generate_tasks":
                        gui.prompt_text.delete(1.0, tk.END)
                        gui.prompt_text.insert(1.0, "Gere tasks baseadas no PRD")
                        gui._send_message()
                    elif action == "generate_n8n":
                        gui.current_agent_type.set("n8n")
                        gui._on_agent_changed()
                        gui.prompt_text.delete(1.0, tk.END)
                        gui.prompt_text.insert(1.0, "Gere um workflow N8N")
                        gui._send_message()
                    elif action == "generate_deploy":
                        gui.current_agent_type.set("deploy")
                        gui._on_agent_changed()
                        gui.prompt_text.delete(1.0, tk.END)
                        gui.prompt_text.insert(1.0, "Gere script de deploy")
                        gui._send_message()
                ...
                agent.voice_module.set_command_callback(voice_command_handler)
